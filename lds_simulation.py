"""
Created on Mar 11 8:41:26 2023
...
class Simulation:
    ...
    def run(self, veh, gvw):
        ...
        p = np.maximum(p, -veh.motor_power) # Remove sections relying on mechanical breaking
        e_operation = sum(p*np.diff(t)) # Calculate Energy Demand
        con = e_operation/3600/s[-1] # Consumption in kWh/km
        v_avg = s[-1]/t[-1]*3.6 # Average speed in km/h
        p = np.append(p, 0) # Append power value to create equal length lists
